...
bool Prism::Load() {
  LOG(INFO) << "loading prism file: " << file_path().u8string();

  if (IsOpen())
    Close();

  if (!OpenReadOnly()) {
    LOG(ERROR) << "error opening prism file '" << file_path().u8string() << "'.";
    return false;
  }
...
bool Prism::Save() {
  LOG(INFO) << "saving prism file: " << file_path().u8string();
  if (!trie_->total_size()) {
...
bool Prism::Build(const Syllabary& syllabary,
  ...
  if (!Create(image_size + estimated_map_size + kReservedSize)) {
    LOG(ERROR) << "Error creating prism file '" << file_path().u8string() << "'.";
    return false;
  }
  // creating metadata
